...
const imagePath = ref(burgerImage);
...
const updateDisplayedCategories = () => {
  if (props.popularCategories.length > 0) {
    displayedCategories.value = props.popularCategories.slice(
      startIndex,
      startIndex + numberOfCategories
    );
    startIndex =
      (startIndex + numberOfCategories) % props.popularCategories.length;
  }
};
...
onMounted(() => {
  ...
  setInterval(updateDisplayedCategories, 10000);
});
...
<template>
  <header class="flex justify-between items-center px-10 py-8">
    ...
    <NuxtLink to="/" class="flex items-center gap-4">
      <img src="public/itl_logo.svg" alt="itl.b2c logo" />
      <div>
        <h2 class="text-base">itl.b2c</h2>
        <p>Готовый интернет-магазин</p>
      </div>
    </NuxtLink>

    <!--Button Catalog-->
    <store-button :imageSrc="imagePath" @click="() => emit('open-modal')"
      >Каталог товаров</store-button
    >
    <!--Search-->
    <div class="flex items-center gap-4 w-6/12 relative">
      <input
        type="text"
        placeholder="Искать товар со скидками до 50% "
        class="border border-blue-400 rounded w-full px-5 py-2 text-blue-400 focus:border-blue-400 outline-blue-500"
      />
      <svg
    ...
    <div class="flex items-center gap-4">
      ...
      >
        <path
          d="M11.4375 2.9375L11.9531 3.5L12.5156 2.98438C14.0625 1.4375 16.2656 0.734375 18.375 1.10938C21.6094 1.625 24 4.4375 24 7.71875V7.95312C24 9.92188 23.1562 11.7969 21.75 13.1094L13.2656 21.0312C12.9375 21.3594 12.4688 21.5 12 21.5C11.4844 21.5 11.0156 21.3594 10.6875 21.0312L2.20312 13.1094C0.796875 11.7969 0 9.92188 0 7.95312V7.71875C0 4.4375 2.34375 1.625 5.57812 1.10938C7.6875 0.734375 9.89062 1.4375 11.4375 2.9375C11.4375 2.98438 11.3906 2.9375 11.4375 2.9375ZM11.9531 6.6875L9.84375 4.53125C8.8125 3.54688 7.35938 3.07812 5.95312 3.3125C3.79688 3.6875 2.25 5.51562 2.25 7.71875V7.95312C2.25 9.3125 2.76562 10.5781 3.75 11.4688L12 19.1562L20.2031 11.4688C21.1875 10.5781 21.75 9.3125 21.75 7.95312V7.71875C21.75 5.51562 20.1562 3.6875 18 3.3125C16.5938 3.07812 15.1406 3.54688 14.1094 4.53125L11.9531 6.6875Z"
          fill="#0E6CDD"
        />
      </svg>
      <span>Избранное</span>
  ...
  <div class="pb-8">
    <ul class="flex gap-4 justify-between px-10 text-blue-500">
      <li
        v-for="category in displayedCategories"
        :key="category.id"
        class="navbar-item"
      >
        <a :href="category.link" class="navbar-link">{{ category.name }}</a>
      </li>
    </ul>
  </div>
</template>
